"""Schema drift policies, snapshots, and helpers exposed via the metadata SDK."""
...
from .utils import safe_upper, to_serializable
...
class SchemaDriftValidator:
    ...
    def _spark_type(self, field: Any) -> Optional[str]:
        ...
        if data_type is None:
            return None
        type_name = getattr(data_type, "simpleString", None)
    ...
    def _types_equivalent(self, expected: Optional[str], observed: Optional[str]) -> bool:
        ...
        if expected == observed:
            return True
        exp_base = expected.split("(", 1)[0].strip()
        obs_base = observed.split("(", 1)[0].strip()
        numeric = {"number", "decimal", "numeric", "bigint", "int", "integer", "double", "float"}
        text = {"string", "varchar", "varchar2", "text", "char", "character varying", "nvarchar"}
        if exp_base in numeric and obs_base in numeric:
            return True
        if exp_base in text and obs_base in text:
            return True
        return False
