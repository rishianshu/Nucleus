...
from runtime_common.events import (
    Emitter,
...
from runtime_common.notification import Notifier
from ingestion_runtime.strategies import ExecutionContext
from runtime_common.orchestrator_helpers import (
    build_heartbeat,
...
from runtime_common.state import BufferedState
from runtime_common.tools.base import ExecutionTool
from runtime_common.tools.spark import SparkTool
from runtime_common.staging import Staging
from runtime_orchestration.metadata_events import MetadataEventSubscriber
try:
    from metadata_service.runtime import collect_metadata, build_metadata_access
    ...
    if _root is not None:
        svc_src = _root / "packages" / "metadata-service" / "src"
        if svc_src.exists():
            sys.path.append(str(svc_src))
            from metadata_service.runtime import collect_metadata, build_metadata_access  # type: ignore  # noqa: E402
        else:
            raise
    else:
        raise
from ingestion_runtime.ingestion import run_ingestion
from ingestion_runtime.orchestration import build_orchestration_plan
from ingestion_runtime.maintenance import run as run_maintenance
...
def main(
    ...
    metadata_gateway = getattr(metadata_access, "gateway", None) if metadata_access else None
    metadata_sdk = getattr(metadata_access, "sdk", None) if metadata_access else None
    metadata_flags = cfg.get("runtime", {}).get("metadata_gateway", {}) or {}
    ...
    if metadata_access or metadata_gateway or metadata_sdk:
        emitter.subscribe(
    ...
    context = ExecutionContext(
        ...
        metadata_sdk=metadata_sdk,
        metadata_gateway=metadata_gateway,
        metadata_feature_flags=metadata_flags,
    )
