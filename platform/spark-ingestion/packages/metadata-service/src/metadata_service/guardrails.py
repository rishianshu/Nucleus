"""Precision guardrail evaluators used by the metadata service."""
        ...
        for field in fields:
            ...
            if precision is None:
                if open_precision_action == "fail":
                    ...
                    fatal = True
                    continue
                fallback_prec = fallback_precision if fallback_precision is not None else precision_limit
                fallback_sc = fallback_scale if fallback_scale is not None else scale
                cast_specs[key] = PrecisionSpec(
                    column=name,
                    precision=fallback_prec,
                    scale=fallback_sc,
                    target_type="numeric",
                )
                issues.append(
                    PrecisionIssue(
                        ...
                        precision=fallback_prec,
                        scale=fallback_sc,
                        reason="precision undefined; using fallback",
                        handled=True,
                        action="fallback_numeric",
                    )
                )
                continue
            if scale is None and fallback_scale is not None:
                cast_specs[key] = PrecisionSpec(
                    column=name,
                    precision=precision,
                ...
                issues.append(
                    PrecisionIssue(
                        ...
                        precision=precision,
                        scale=fallback_scale,
                        reason="scale undefined; applying fallback",
                        handled=True,
                        action="fallback_scale",
                    )
                )
                continue
        if fatal:
