        ...
        for data in raw_constraints.values():
            ...
            constraints.append(
                DatasetConstraint(
                    ...
                    bad=data.get("bad"),
                    rely=data.get("rely"),
                    novalidate=data.get("novalidate"),
                    view_related=data.get("view_related"),
                    columns=fields,
...
def _extract_comment(row: Optional[Dict[str, Any]]) -> Optional[str]:
...
__all__ = ["OracleMetadataNormalizer"]
