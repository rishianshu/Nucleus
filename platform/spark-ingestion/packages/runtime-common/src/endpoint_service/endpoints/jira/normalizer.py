...
class JiraMetadataNormalizer(MetadataNormalizer):
    ...
    def normalize(
        ...
        *,
        ...
        snapshot = CatalogSnapshot(
            ...
            table=dataset_cfg.get("table") or dataset.name or dataset.id or "jira_dataset",
    ...
    def _build_constraints(self, constraints: List[Dict[str, Any]]) -> List[DatasetConstraint]:
        ...
        for constraint in constraints:
            ...
            fields = [
                DatasetConstraintField(
                    field=field.get("field") or field.get("name") or "",
                    position=_optional_int(field.get("position")),
                )
                for field in constraint.get("fields", [])
                if field.get("field") or field.get("name")
            ]
            built.append(
                DatasetConstraint(
                    name=name,
