...
from endpoint_service.common import PrintLogger, RUN_ID
from endpoint_service.io.filesystem import HDFSOutbox
...
class FileQueueSubscriber(Subscriber):
    """
    Appends events as JSON lines to a local file path.

    Useful for piping transient events into a staging area or external watcher.
    """

    def __init__(self, path: str, job_name: str) -> None:
        self.path = Path(path)
    ...
    def on_event(self, event: Event) -> None:
        ...
        try:
            ...
            # Swallow errors to avoid interrupting emitters
            return
