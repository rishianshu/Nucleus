...
from runtime_common.io.filesystem import HDFSOutbox
from runtime_common.events import emit_log
from runtime_common.notification import Heartbeat
...
def validate_config(cfg: Dict[str, Any]) -> None:
    ...
    runtime_md = runtime.get("metadata_gateway")
    if runtime_md is None:
        runtime_md = {"ingestion_metrics": False, "ingestion_runtime": False}
        runtime["metadata_gateway"] = runtime_md
    elif not isinstance(runtime_md, dict):
        raise ValueError("runtime.metadata_gateway must be an object when provided")
    else:
        for key in ["ingestion_metrics", "ingestion_runtime"]:
            runtime_md.setdefault(key, False)
