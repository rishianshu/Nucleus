...
from runtime_common.common import RUN_ID
...
def _default_exec_command(args: Optional[Iterable[str]] = None) -> List[str]:
    base = ["spark-submit", "--py-files", "dist/ingestion_runtime_bundle.zip", "ingestion.py"]
    if args:
...
def _build_plain_cron(cfg: Dict[str, Any], args: Optional[Iterable[str]]) -> OrchestrationPlan:
    ...
    metadata = {
        "example_crontab": f"{schedule['expression']} ingestion_runtime_runner.sh",
