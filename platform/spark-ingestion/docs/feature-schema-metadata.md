...
- Align with the cross-system contracts defined in `docs/metadata-platform.md` so ingestion, reconciliation, and future services can emit and consume records consistently.
...
- Storage: File-based (JSON/Parquet) or lightweight embedded DB (SQLite) under `metadata/records/{kind}/{schema.table}/{timestamp_run}.json`, matching the layout described in `docs/metadata-platform.md`. Consider pluggable backends for shared storage (S3, GCS) later.
...
- Client Libraries: Utilities inside `ingestion_runtime` to read cached metadata, detect drift, and feed planners or ingestion pipelines. Future work exposes the same artifacts over gRPC while keeping file-path access for lightweight consumers.
...
- Define configuration surface (`catalog.ttl_hours`, `catalog.storage_path`, `catalog.refresh_on_startup`) and integration points in `ingestion_runtime`.
