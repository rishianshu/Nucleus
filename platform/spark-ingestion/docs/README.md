...
```bash
python3 scripts/build_zip.py --output dist/ingestion_runtime_bundle.zip \
    --include conf --include db
```

Key options:

- `--output`: target zip path (default `dist/ingestion_runtime_bundle.zip`).
- `--include`: extra top-level directories to embed in the archive (repeatable).

The script always packages `ingestion.py` and the entire `ingestion_runtime` package. Run `python3 scripts/build_zip.py --help` for the full option list.

### Submitting the job

```bash
spark3-submit \
  --py-files dist/ingestion_runtime_bundle.zip \
