...
from ingestion_models.schema import SchemaDriftPolicy, SchemaDriftResult, SchemaValidationError, build_schema_snapshot

from endpoint_service.common import with_ingest_cols
...
from metadata_service.ingestion.planning.base import Planner, PlannerRequest
from endpoint_service.io.paths import Paths
from endpoint_service.common import Utils, PrintLogger, RUN_ID
from endpoint_service.tools.base import ExecutionTool
from endpoint_service.events import emit_state_mark, emit_state_watermark, emit_log
...
class ExecutionContext:
    ...
    def emit_event(self, category, event_type, **payload):
        if self.emitter is None:
            return
        from endpoint_service.events.types import Event
    ...
    def _add_literal_column(self, dataframe: Any, column: str, value: Any = None) -> Any:
        if self.tool and hasattr(self.tool, "with_literal_column"):
            try:
                ...
                return dataframe
        if hasattr(dataframe, "withColumn"):
            try:
                return dataframe.withColumn(column, value)
            except Exception:
                return dataframe
