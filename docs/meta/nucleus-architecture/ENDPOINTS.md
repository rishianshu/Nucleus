...
| `http.onedrive` | HTTP / Microsoft OneDrive (Graph) | `runtime_common/endpoints/onedrive/onedrive_http.py` | `metadata`, `preview`, `ingestion` | `endpoint_service.endpoints.onedrive.metadata.OneDriveMetadataSubsystem` + `normalizer.py` (docs dataset; API catalog lives in `onedrive_catalog.py`) | Stub-friendly Graph harness; enumerates docs under a configured drive/root and exposes a docs ingestion unit with CDM mapping. |
