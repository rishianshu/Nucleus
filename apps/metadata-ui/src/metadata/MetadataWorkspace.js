...
function isConfluenceDataset(dataset) {
    if (!dataset) {
        return false;
    }
    ...
    return Boolean(dataset.labels?.some((label) => typeof label === "string" && label.toLowerCase().includes("confluence")));
}
function extractConfluenceExcerpt(value) {
...
export function MetadataWorkspace({ metadataEndpoint, authToken, projectSlug, userRole, datasetDetailRouteId, onDatasetDetailRouteChange, }) {
    ...
    const renderCatalogSection = () => {
        ...
        const confluencePreviewRows = isConfluencePreview
    ...
    const toastPortal = toastQueue.toasts.length ? (_jsx("div", { className: "pointer-events-none fixed bottom-4 right-4 z-50 flex justify-end px-4 sm:px-6", children: _jsx("div", { className: "flex w-full max-w-sm flex-col gap-2", children: toastQueue.toasts.map((toast) => {
